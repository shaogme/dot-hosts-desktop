...
let
  ...
  # 从 npins 锁定源获取官方发布包
  unpacked = sources.firefox-developer-edition;
  ...
  fhs = pkgs.buildFHSEnv {
    name = "firefox-developer-edition-fhs";
    ...
    extraBwrapArgs = [
      ...
      "--bind" "\${XDG_DATA_HOME:-$HOME/.local/share}/sandboxes/firefox-developer-edition" "$HOME"
    ...
    runScript = pkgs.writeShellScript "firefox-developer-edition-launcher" ''
      # 适配 Wayland 桌面环境
      ...
      # 启动 Firefox Developer Edition 主程序
      exec ${unpacked}/firefox "$@"
    '';
  };

  # 宿主机包装脚本：提前创建独立的持久化沙箱目录，然后启动隔离容器
  wrapper = pkgs.writeShellScriptBin "firefox-developer-edition" ''
    SANDBOX_HOME="''${XDG_DATA_HOME:-$HOME/.local/share}/sandboxes/firefox-developer-edition"
    mkdir -p "$SANDBOX_HOME/Downloads" "$SANDBOX_HOME/.mozilla"

    exec ${fhs}/bin/firefox-developer-edition-fhs "$@"
  '';
  ...
  desktopItem = pkgs.makeDesktopItem {
    ...
    exec = "firefox-developer-edition %U";
...
in
pkgs.symlinkJoin {
  name = "firefox-developer-edition-${version}";
  paths = [
    wrapper
    desktopItem
  ];
  postBuild = ''
    # 提供 firefox-devedition 软链接别名
    ln -s firefox-developer-edition $out/bin/firefox-devedition
    ...
    for size in 16 32 48 64 128; do
      ...
      if [ -f "$icon_file" ]; then
        ...
        cp "$icon_file" "$out/share/icons/hicolor/''${size}x''${size}/apps/firefox-developer-edition.png"
        cp "$icon_file" "$out/share/icons/hicolor/''${size}x''${size}/apps/firefox-devedition.png"
      fi
    done
  '';
}
